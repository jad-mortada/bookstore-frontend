...
import { Box, Typography, MenuItem, TextField, Checkbox, Snackbar, Alert, Tooltip } from '@mui/material';
...
const ListBooksManagement = () => {
  ...
  useEffect(() => {
    if (selectedList && !isNaN(Number(selectedList))) {
      setLoading(true);
      listService.getListBooks(Number(selectedList)).then(res => setLinkedBooks(res.data || [])).catch(() => setLinkedBooks([])).finally(() => setLoading(false));
    } else {
      setLinkedBooks([]);
    }
  ...
  const handleToggle = async (bookId) => {
    ...
    try {
      if (linkedBooks.some(b => b.bookId === bookId)) {
        ...
        setSnackbar({ open: true, message: 'Book unlinked!', severity: 'success' });
      } else {
        await listService.linkBookToList(selectedList, bookId);
        setSnackbar({ open: true, message: 'Book linked!', severity: 'success' });
      }
      // Refresh
      try {
  ...
  // Helpers for display
  const getClassById = (classId) => classes.find(c => String(c.id) === String(classId));
  const getSchoolById = (schoolId) => schools.find(s => String(s.id) === String(schoolId));
  const getListDisplay = (list) => {
    ...
    const schoolDisplay = sameNameSchools.length > 1 ? `${school.name} (${school.address})` : school.name;
