...
const CustomerOrderFlow = () => {
   ...
   */
  const getRowKey = (row) =>
    String(row?.bookId ?? row?.id ?? row?.book?.id ?? row?.listBookId ?? "");
  ...
  const getBasePrice = (row) =>
    Number(row.price ?? row.bookPrice ?? row.unitPrice ?? 0);
  ...
  const getUnitPrice = (row) =>
    row.conditionType === "USED" ? getBasePrice(row) * 0.5 : getBasePrice(row);
  ...
  const fetchList = async () => {
    try {
      ...
      const res = await customerBookOrderService.getOfficialList(
        selectedClass,
        year
      );
      let listBooks = [];
      ...
      if (Array.isArray(res.data)) {
        ...
        const match = res.data.find(
          (l) => Number(l.classId) === cid && Number(l.year) === yr
        );
        if (match?.id) {
      ...
      try {
        ...
        const needsEnrichment = (listBooks || []).some(
          (b) => !b?.imageUrl && !(b?.book && b.book.imageUrl)
        );
        ...
        if (needsEnrichment) {
          ...
          listBooks = (listBooks || []).map((book) => {
            ...
            const keyTitle = String(book?.bookTitle || book?.title || "")
              .toLowerCase()
              .trim();
            const match =
              byId.get(String(book?.bookId ?? book?.id)) ||
      ...
      const scName =
        schoolsArr.find((s) => String(s.id) === String(selectedSchool))?.name ||
        String(selectedSchool);
      const clName =
        classesArr.find((c) => String(c.id) === String(selectedClass))?.name ||
      ...
      if (status === 403) {
        message =
          "You do not have permission to view lists. Please sign in as a customer.";
      } else if (status === 404) {
  ...
  const handleAddCurrentSelection = () => {
    ...
    if (!currentItems.length) {
      setSnackbar({
        open: true,
        message: "Load a list first before adding.",
        severity: "warning",
      });
      return;
    }
    ...
    if (!selectedIds.length) {
      setSnackbar({
        open: true,
        message: "Select at least one book using the checkboxes.",
        severity: "warning",
      });
      return;
    }
    ...
    const chosen = currentItems.filter((it) =>
      selectedIds.includes(getRowKey(it))
    );
    ...
    setSnackbar({
      open: true,
      message: "Selected books added to order.",
      severity: "success",
    });
  ...
  const handleAddAllCurrentSelection = () => {
    ...
    setSnackbar({
      open: true,
      message: "Entire list added to order.",
      severity: "success",
    });
  ...
  const handleSubmitOrder = async () => {
    try {
      ...
      const itemsToSubmit = accumulatedItems.length
        ? accumulatedItems
        : currentItems;
      ...
      if (status === 400) {
        ...
        message =
          "Book already exists in this draft. You can modify the quantity instead.";
      } else if (serverMsg) {
        // Use server-provided message if available
        message = serverMsg;
      }
      ...
      >
        {noSchools && (
          <GlassCard elevation={6} sx={{ borderRadius: "1vw", mb: "2vw" }}>
            <CardHeader
              title={<Typography variant="h6">Ordering Unavailable</Typography>}
            />
            <CardContent>
              <Typography variant="body1">
                The ordering workflow is currently unavailable because no
                schools are set up. Please check back later or contact support.
              </Typography>
            </CardContent>
          </GlassCard>
        )}
        <GlassCard elevation={6} sx={{ borderRadius: 3, mb: 3 }}>
          <CardHeader
            title={<Typography variant="h5">Place Order</Typography>}
          />
          <CardContent>
            <Box
              sx={{
            ...
            >
              <Autocomplete
                ...
                value={
                  toArray(schools).find(
                    (s) => String(s.id) === String(selectedSchool)
                  ) || null
                }
                onChange={(_, val) =>
                  setSelectedSchool(val?.id ? String(val.id) : "")
                }
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    sx={{ width: { xs: "100%", sm: "45vw", md: "28vw" } }}
                    slotProps={{
                      input: {
                        ...(params?.InputProps || {}),
                        endAdornment: (
              ...
              <Autocomplete
                ...
                value={
                  toArray(classes).find(
                    (c) => String(c.id) === String(selectedClass)
                  ) || null
                }
                onChange={(_, val) =>
                  setSelectedClass(val?.id ? String(val.id) : "")
                }
                renderInput={(params) => (
                  <TextField
                    {...params}
                    ...
                    sx={{ minWidth: { xs: "60vw", sm: "40vw", md: "24vw" } }}
              ...
              <Tooltip title="Add all books from the current list to your order">
                <span>
                  <Button
                    variant="outlined"
                    onClick={handleAddAllCurrentSelection}
                    aria-label="Add entire list"
        ...
        {officialList.length > 0 && (
          <GlassCard elevation={6} sx={{ borderRadius: 3, mb: 3 }}>
            <CardHeader
              title={<Typography variant="h6">Official Book List</Typography>}
            />
            <CardContent>
              <Box
                sx={{
                ...
                <Stack direction="row" spacing={1}>
                  <Tooltip title="Select all books in the list">
                    <Button
                      ...
                      onClick={() =>
                        setSelectedIds(currentItems.map((i) => getRowKey(i)))
                      }
                    >
                      Select All
                    </Button>
            ...
            <CardHeader
              title={
                <Typography variant="h6">
                  Items in Your Order ({accumulatedItems.length})
                </Typography>
              }
            />
            <CardContent>
                        ...
                        >
                          <Tooltip title="Remove item">
                            <IconButton
                              ...
                              onClick={() => handleRemoveAccumulated(idx)}
                            >
              ...
              >
                ...
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  <Typography variant="subtitle1">
                    ...
                    <b>
                      {currency(
                        accumulatedItems.reduce(
                          (sum, it) => sum + getSubtotal(it),
                          0
                        )
                      )}
                    </b>
        ...
        {accumulatedItems.length === 0 && (
          <GlassCard elevation={4} sx={{ borderRadius: "1vw", mb: "2vw" }}>
            <CardContent sx={{ textAlign: "center", py: "6vh" }}>
              ...
              <Typography variant="body2" color="text.secondary">
                Browse the official list, select books, and click "Add Selected"
                to build your order.
              </Typography>
            </CardContent>
          </GlassCard>
