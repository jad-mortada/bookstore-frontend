...
import { Typography, Box, Snackbar, Alert, Button, Dialog, DialogTitle, DialogContent, DialogActions, IconButton, Chip, Divider, Tooltip, TextField } from '@mui/material';
  ...
  const fetchOrders = React.useCallback(async () => {
    ...
    try {
      ...
      const mapped = raw.map(order => {
        ...
        return {
          ...order,
          customer: [order.customerFirstName, order.customerLastName].filter(Boolean).join(' '),
    ...
    { field: 'createdAt', headerName: 'Created At', width: 160 },
    {
      ...
      renderCell: (params) => (
        <Tooltip title={`Delete order #${params?.row?.id ?? ''}`}>
          <IconButton onClick={(e) => { e.stopPropagation(); setDeleteOrderId(params.row.id); }} size="small" color="error" aria-label={`Delete order ${params?.row?.id ?? ''}`}>
  ...
  return (
    <BackgroundFX>
      <Box sx={{ p: { xs: 1, sm: 2, md: 3 }, maxWidth: '100%', mx: 0 }}>
        <GlassCard elevation={6} sx={{ borderRadius: 3, mb: 2, p: { xs: 2, md: 3 } }}>
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2, mb: 2, flexWrap: 'wrap' }}>
            <Typography variant="h5" sx={{ fontWeight: 800, mr: 'auto' }}>Customer Orders</Typography>
            <TextField
              size="small"
              label="Search orders"
          ...
          {(!loading && (Array.isArray(orders) ? orders.length === 0 : true)) ? (
            <GlassCard elevation={0} sx={{ p: 4, textAlign: 'center' }}>
              <Typography variant="h6" sx={{ fontWeight: 700, mb: 1 }}>No orders yet</Typography>
              <Typography variant="body2" color="text.secondary">Your orders will appear here once you place one.</Typography>
            </GlassCard>
          ) : (
            <GlassCard elevation={0} sx={{ p: 0 }}>
              <DataGrid
                rows={filteredOrders}
                columns={columns}
                pageSize={10}
                ...
                loading={loading}
                getRowId={(row) => row?.id ?? row?._id ?? `${row?.customer || 'order'}-${row?.createdAtRaw || ''}`}
        ...
        <Dialog open={!!detailOrder} onClose={() => setDetailOrder(null)} fullWidth maxWidth="md">
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5, mb: 2 }}>
                  ...
                  {detailOrder.customer && (
                    <Chip size="small" variant="outlined" label={`Customer: ${detailOrder.customer}`} />
                  )}
                  {detailOrder.school && (
                    <Chip size="small" variant="outlined" label={`School: ${detailOrder.school}`} />
                  )}
                  {detailOrder.class && (
                    <Chip size="small" variant="outlined" label={`Class: ${detailOrder.class}`} />
                  )}
                  {((detailOrder.orderItems || [])[0]?.year) && (
                    <Chip size="small" variant="outlined" label={`Year: ${String((detailOrder.orderItems || [])[0].year)}`} />
                  )}
                </Box>
                ...
                {/* Items Table */}
                <Box sx={{ mt: 2 }}>
                  <Typography variant="h6" sx={{ mb: 1 }}>Books in Order</Typography>
                    ...
                    >
                      ...
                      <Box component="tbody">
                        {(detailOrder.orderItems || []).map((item, idx) => {
                          ...
                          const hasExplicitUnit = !Number.isNaN(rawExplicit) && item.unitPrice !== null && item.unitPrice !== undefined;
                          const approxEq = (a, b) => Math.abs(Number(a) - Number(b)) <= 0.005;
                          const expectedUsed = basePrice * 0.5;
                          const normalizedExplicit = (() => {
                            if (!hasExplicitUnit) return null;
                            const perUnitCandidate = rawExplicit / qty;
                            if (isUsed) {
                              if (approxEq(rawExplicit, expectedUsed)) return rawExplicit;
                              if (approxEq(perUnitCandidate, expectedUsed)) return perUnitCandidate;
                              if (qty > 1) return perUnitCandidate;
                              return rawExplicit;
                            } else {
                              if (approxEq(rawExplicit, basePrice)) return rawExplicit;
                              if (approxEq(perUnitCandidate, basePrice)) return perUnitCandidate;
                              if (qty > 1) return perUnitCandidate;
                              return rawExplicit;
                            }
                          })();
                          const unitPrice = (normalizedExplicit != null) ? normalizedExplicit : (isUsed ? expectedUsed : basePrice);
                          const displayBase = isUsed ? ((normalizedExplicit != null) ? (normalizedExplicit / 0.5) : basePrice) : basePrice;
                          ...
                          return (
                            <motion.tr key={item.id || `${item.bookId || item.id || 'row'}-${idx}`} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
                              ...
                              <Box component="td" sx={{ p: 1.25, textAlign: 'right' }}>
                                {isUsed && (
                                ...
                                {isUsed && <Box sx={{ fontSize: '0.75em', color: (t) => t.palette.success.main }}>(50% off)</Box>}
                              </Box>
                              <Box component="td" sx={{ p: 1.25, textAlign: 'center' }}>{quantity}</Box>
                              <Box component="td" sx={{ p: 1.25, textAlign: 'right', fontWeight: 500 }}>${subtotal.toFixed(2)}</Box>
                            </motion.tr>
                          );
                        })}
                      </Box>
